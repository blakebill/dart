...
- Page, section, body and caption text use the matching `--font-*` tokens.
...
- At 1280x720, the overview must not scroll vertically.
- At 960px wide, content must not overflow horizontally.
...
- Settings controls stay equal-width; the unified save bar remains reachable
  and may stack on narrow layouts.
...
subscriptions or runtime data. The visual suite also asserts overflow,
dashboard height, settings control widths and tool grid columns.
